...
const inputDockMeasuredHeight = ref<number | null>(null);
...
const bodyPanelStyle = computed(() => {
  ...
  if (inputDockMeasuredHeight.value != null) {
    style['--input-panel-height'] = `${inputDockMeasuredHeight.value}px`;
  }
...
function persistSidePanelTab(value: SidePanelTab) {
...
function selectSidePanelTab(value: SidePanelTab) {
  if (sidePanelActiveTab.value === value) {
...
function syncFloatingExtent() {
  ...
  if (input) {
    ...
    if (height > 0) inputDockMeasuredHeight.value = height;
  }
...
function updateFloatingExtentObserver() {
  ...
  const nextInput = inputEl.value;
  if (floatingExtentObservedInput && floatingExtentObservedInput !== nextInput) {
    floatingExtentResizeObserver.unobserve(floatingExtentObservedInput);
  }
  if (nextInput && nextInput !== floatingExtentObservedInput) {
    floatingExtentResizeObserver.observe(nextInput);
  }
  floatingExtentObservedInput = nextInput ?? null;
...
function showBrowserNotification(
  ...
  const notification = new Notification(`${kind}`, {
    body,
    tag: `hex-${type}-${projectId}-${sessionId}`,
  });
...
watch(uiTheme, (theme) => {
  ...
  fw.entries.value.forEach((entry) => {
    if (
      ...
      entry.key.startsWith('reasoning:') ||
      entry.key.startsWith('history-reasoning:')
    ) {
...
async function handleEditMessage(payload: {
  ...
  try {
    ...
    attachments.value = (payload.attachments ?? []).map((item) => ({
      id: generateAttachmentId(),
      filename: item.filename,
      mime: item.mime,
      dataUrl: item.dataUrl,
    ...
    sessionError.value = `Message edit failed: ${toErrorMessage(error)}`;
  }
}
...
onMounted(() => {
  ...
  const chosenTheme =
    uiTheme.value === 'light' && availableThemes.includes('github-light')
      ? 'github-light'
      : pickShikiTheme(availableThemes);
...
onBeforeUnmount(() => {
  ...
  floatingExtentObservedInput = null;
...
.app-input {
  ...
  width: 100%;
  height: auto;
  min-height: var(--input-panel-default);
