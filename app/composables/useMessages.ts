...
function removeRootsFrom(rootId: string) {
  const rootIndex = roots.value.findIndex((root) => root.id === rootId);
  if (rootIndex < 0) return;

  const messageIds = new Set<string>();
  for (const root of roots.value.slice(rootIndex)) {
    for (const message of getThread(root.id)) {
